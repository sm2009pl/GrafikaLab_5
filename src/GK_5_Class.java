 ...
 * stereo.
 */
public class GK_5_Class extends GLJPanel implements GLEventListener, KeyListener{
    ...
    public static void main(String[] args) {
        ...
        GK_5_Class panel = new GK_5_Class();
    ...
    private void draw(GL2 gl2) {
        ...
        switch(objectNumber) {
            case 1:
                drawKorkociag(gl2, 5, 20, 10.0f);
                break;
            case 2:
                drawPiramida(gl2, 10, 4);
                break;
        }

    }
    private void drawKorkociag(GL2 gl2, int repeat, int sides, float size)
    {
        float r = 1;
        float height = 1;
        for(int j = 0; j < repeat; j++)
        {
            for(int i = 0; i < sides; i++)
            {
                ...
                {
                    gl2.glColor3f( 0.5f, 0.25f, 0.0f );
                }

                gl2.glBegin( gl2.GL_POINTS );
                double kat = i * 2 * Math.PI / sides;
                gl2.glVertex3f((float)(r * Math.cos(kat)),(float)(r * Math.sin(kat)),height);
                kat = (i + 1) * 2 * Math.PI / sides;
                gl2.glVertex3f((float)(r * Math.cos(kat)),(float)(r * Math.sin(kat)),height);
                gl2.glEnd();
                height -= 0.1f;
                size -= (10.0f/(repeat+sides));
            }
        }
    }

    private void drawPiramida(GL2 gl2, int sides, float prom)
    {
        ...
        for(int i = 0; i < sides; i++)
        {
            kat = i * 2 * Math.PI / sides;
            gl2.glVertex3f((float)(prom * Math.cos(kat)),(float)(prom * Math.sin(kat)),0.0f);
            kat = (i + 1) * 2 * Math.PI / sides;
            gl2.glVertex3f((float)(prom * Math.cos(kat)),(float)(prom * Math.sin(kat)),0.0f);
